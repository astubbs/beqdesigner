...
# Real-audio and synthetic feature extraction use shared helpers from conftest_nn.
...
def test_extract_and_validate(tmp_path):
    ...
    for t in extracted:
        ...
        if not entry.get("filters"):
            continue
        try:
            real_feats = extract_real_audio_features(t["wav_path"], DEFAULT_GRID, _DEFAULT_FS)
        except Exception as exc:
            log.warning("feature extraction failed for %s: %s", t["title"], exc)
            continue
        synth_feats = synthetic_features(entry, DEFAULT_GRID)
    ...
    X_train, Y_train, _ = build_held_out_training_set(tmdb_cache, val_tmdb_ids)
    ...
    for name, model in [
        ...
        real = compute_mean_loss(model, X_val_real, val_entries)
        synth = compute_mean_loss(model, X_val_synth, val_entries)
