"""Unit tests for helpers in ``scripts/generate_beq_profile.py``.

The script is not normally importable as a module (it lives outside the
``src/main`` source tree), so we load it via ``importlib.util``.  Only
pure-Python helpers are exercised here — anything that calls ffprobe,
...
_SCRIPT_PATH = _REPO_ROOT / "scripts" / "generate_beq_profile.py"


@pytest.fixture(scope="module")
def gen_module():
    spec = importlib.util.spec_from_file_location(
        "generate_beq_profile", _SCRIPT_PATH,
    )
