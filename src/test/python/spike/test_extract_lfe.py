"""Unit tests for ``scripts/extract_lfe.py`` — bias-corrected unmatched selection.

The extractor lives outside the ``src/main/python`` source tree so we
load it via ``importlib.util``. Only pure-function helpers are tested
here — anything that actually shells out to ffmpeg belongs in an
integration test.
"""
...
_SCRIPT_PATH = _REPO_ROOT / "scripts" / "extract_lfe.py"


@pytest.fixture(scope="module")
def extract_mod():
    """Load extract_lfe.py as a module without executing its main()."""
    spec = importlib.util.spec_from_file_location(
        "extract_lfe", _SCRIPT_PATH,
    )
