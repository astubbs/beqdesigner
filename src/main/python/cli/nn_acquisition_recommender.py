...
def _load_library_inventory(
    inventory_path: Path | None,
) -> tuple[set[str], list[str]]:
    """Read media_inventory.json and return (tmdb IDs already owned, missing-ID files).

    The tmdb IDs are titles already in the user's library and so should
    ...
    **Fail-fast semantics**: if the caller explicitly passes an
    ...
    if not inventory_path.exists():
        ...
        print(
            ...
            "Run bin/beq-designer extract to refresh the inventory.",
            file=sys.stderr,
        )
        return set(), []
    try:
    ...
    print(
        ...
        f"{len(missing_ids)} files missing IDs",
        file=sys.stderr,
    )
    return library_tmdb, missing_ids
...
def generate_report(n: int, output=None, inventory_path: Path | None = None) -> None:
    ...
    if library_tmdb_ids:
        ...
        pr(f"**Media library inventory**: not available — recommendations "
           f"may include titles you already own. Run `bin/beq-designer extract` "
    ...
    # Missing IDs section: media library files without a tmdb tag.
    # These files are excluded from extraction; renaming them to add the
    # tag would let the model learn from them.
    if missing_ids:
        pr("## Library files needing DB IDs")
        pr()
        pr(f"**{len(missing_ids):,} media files** in your library are "
           "missing a `[tmdb-NNN]`, `[tvdb-NNN]`, or `[imdb-NNN]` tag in "
           "the filename.  These are excluded from extraction.  Renaming "
           "them to include the correct tag would let the model learn "
           "from them — these may already match the catalogue and so are "
           "free training data once tagged.")
        pr()
        ...
        if len(missing_ids) > 30:
            pr(f"... and {len(missing_ids) - 30:,} more (see `media_inventory.json`).")
            pr()

    pr("---")
    pr("Generated by `bin/beq-designer acquisition-report`")
