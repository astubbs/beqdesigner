"""Pure data classes for frequency-domain audio data.

Qt-free -- safe to import from CLI scripts and headless Docker containers.
MagnitudeData and ComplexData were extracted from model/xy.py to break
the import chain: model/iir.py -> model/xy.py -> model/preferences.py -> PyQt6.

GUI code should continue importing from model/xy (which re-exports these).
CLI/model code should import from model/xy_data to avoid Qt.
"""
...
# ---------------------------------------------------------------------------
# Smoothing (pure computation -- no Qt)
# ---------------------------------------------------------------------------
...
class MagnitudeData:
    ...
    def normalise(self, target):
        ...
        '''
        # Late import to avoid Qt dependency -- interp() lives in model/xy
        # which depends on Preferences/QSettings for smoothing preference.
        from model.xy import interp
